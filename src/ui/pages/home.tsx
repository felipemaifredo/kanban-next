...
export const HomePage = () => {
    ...
    const [loading, setLoading] = useState(true) // Para controlar quando os dados estão prontos

    useEffect(() => {
        // Somente no cliente: resgata as listas do localStorage
    ...
    useEffect(() => {
        if (!loading) {
            // Somente atualiza localStorage quando os dados forem carregados
            localStorage.setItem("lists", JSON.stringify(lists))
        }
    }, [lists, loading]) // Atualiza sempre que as listas forem alteradas e não estiver carregando
    ...
    if (loading) {
        return "" // Ou uma animação de carregamento
    }
